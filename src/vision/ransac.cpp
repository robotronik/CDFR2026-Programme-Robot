...
bool findGroupRANSAC2D(
    ...
    for (int iter = 0; iter < maxIterations; ++iter) {
        ...
        for (const auto& p : points) {
            ...
            // filtre orientation
            float diff = angleDiff(p.a, lineAngle);// à tester angle tag pas dans l'axe de l'objet
        ...
        for (size_t k = 0; k + max_blocks - 1 < inliers.size(); k++) {
            ...
            if (status) {
                dx = inliers[0].second->x - inliers[max_blocks - 1].second->x;
                dy = inliers[0].second->y - inliers[max_blocks - 1].second->y;

                norm = std::sqrt(dx*dx + dy*dy);
                if (norm < 1e-6f) continue;

                line;
                line.x = inliers[0].second->x;
                line.y = inliers[0].second->y;
                line.dx = dx / norm;
                line.dy = dy / norm;

                // angle de la ligne (en degrés)
                float angle = std::atan2(line.dy, line.dx) * 180.0f / M_PI;

                for(size_t _ = 0 ; _ < max_blocks; _++){
                    bestGroup.push_back(*inliers[_].second);
                    bestGroup.back().a = angle + 90;
                }
